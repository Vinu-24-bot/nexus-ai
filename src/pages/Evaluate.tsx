...
// 🛡️ THE NUCLEAR FIX: Hard-coded router. Completely ignores Vercel's buggy .env cache.
...
export default function EvaluatePage() {
  ...
  // Check health on mount (helps wake up Render)
  useEffect(() => {
  ...
  const handleGenerateJD = async () => {
    ...
    const templateKey = Object.keys(JD_TEMPLATES).find((k) => posLower.includes(k) || k.includes(posLower));
    ...
    try {
      ...
      setJobDescription(generateLocalJD(targetPosition));
  ...
  const handleGenerateLink = async () => {
    ...
    try {
      ...
      if (err.message === "Failed to fetch") {
         toast.error("⏳ Render Backend is waking up from sleep mode (takes ~50 seconds). Please wait a moment and click Generate again.", { duration: 6000 });
      } else {
