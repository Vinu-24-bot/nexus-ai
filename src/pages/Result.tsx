...
// 🛡️ THE MASTER PLAYER: Flawless Native Math + 404 Error Detection
const ForgeProVideoPlayer = ({ src }: { src: string }) => {
  ...
  const [hasError, setHasError] = useState(false); // Detects if Render deleted the file
  ...
  const skip = (amount: number) => {
    if (hasError || !videoRef.current) return;
    // Pure, unblocked math for flawless skipping
  ...
  // If Render wiped the disk, show this beautiful error state
  if (hasError) {
    return (
      <div className="relative group w-full aspect-video bg-muted/30 rounded-lg overflow-hidden flex flex-col items-center justify-center shadow-inner border border-destructive/30 border-dashed">
        ...
        <p className="text-xs text-muted-foreground mt-1 max-w-sm text-center px-4">
          This recording was deleted from the server storage (likely due to a backend redeployment on a free tier host).
        </p>
      </div>
    );
  }
  ...
  return (
    <div ref={containerRef} className="relative group w-full bg-black rounded-lg overflow-hidden flex flex-col items-center justify-center shadow-inner border border-border/50">
      ...
      <video 
        ...
        onError={() => setHasError(true)} // Catch missing files instantly
        preload="metadata"
      />
...
export default function ResultPage() {
  ...
  // Point the player to the static uploads folder (safest for standard MP4/WebM hosting)
    ...
    <div className="min-h-screen bg-background nexus-grid pb-20">
      ...
      <div className="container mx-auto px-6 pt-24 pb-16 max-w-5xl">
        <motion.div initial="hidden" animate="visible" className="space-y-8">
          ...
          {/* Full-Width Session Recording UI with 404 Error Catcher */}
          {videoFiles.length > 0 && (
