...
export default function DashboardPage() {
  ...
  const fetchData = useCallback(async (showRefresh = false) => {
    ...
    try {
      ...
      // ─── ENTERPRISE FEATURE: Auto-fetch Debrief Matrix if we have candidates ───
      if (data && data.length >= 2) {
        // Just compare the top 5 most recent candidates for the dashboard preview
        ...
        <motion.div initial="hidden" animate="visible" className="space-y-8">
          ...
          <motion.div variants={fadeUp} custom={0} className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <Button variant="outline" size="sm" onClick={() => fetchData(true)} disabled={refreshing} className="h-8">
          ...
          {/* ─── ENTERPRISE UPGRADE: Executive Stats Grid ─── */}
          ...
          {/* ─── ENTERPRISE UPGRADE: Debrief Matrix ─── */}
          {debriefMatrix && debriefMatrix.enterprise_debrief_matrix && (
             <motion.div variants={fadeUp} custom={1.5} className="glass rounded-xl overflow-hidden border border-primary/20">
          ...
          {/* Recent Evaluations List (Existing UI, just cleaned up) */}
