...
export default function DashboardPage() {
  ...
  // 🛡️ THE FIX: Strictly filter using explicitly controlled 'remarks' payload text generated by the L1 Upload module
  const l1TechRoundData = results.filter(r => r.remarks && r.remarks.includes("Pre-recorded interview video uploaded directly from candidate or recruiter dashboard"));
  const initialScreeningData = results.filter(r => !r.remarks || !r.remarks.includes("Pre-recorded interview video uploaded directly from candidate or recruiter dashboard"));
